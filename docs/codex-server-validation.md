...
2026-09-22，在原来的 AutoDL 容器中新增 Codex 机器人，使用 Codex 0.155.1 和 cc-connect 1.5.0。它有独立的飞书应用、项目和状态目录，Claude / DeepSeek 服务继续运行。
...
- 测试拒绝操作时，在飞书回复“拒绝”，工具取消，agent 停止且目标文件不存在。
- 测试单次允许时，先确认文件不存在，再在飞书只回复“允许”后工具执行成功；服务器核验文件内容逐字节一致。飞书显示完成回复。
...
默认 Bubblewrap 沙箱没能启动，因为容器不允许创建所需的 namespace。模型返回了回答，工具却没有成功执行。这个例子也说明，即使 CLI 退出码为 0，仍要核对文件读取结果。
...
当前通过测试的方式是默认只读，写入时逐次批准具体命令。原生 Codex 发起审批，cc-connect 把权限请求发到飞书，收到用户决定后再执行。通过这一测试不代表 `workspace-write` 沙箱已经修复，也不代表未审批命令可以写入。此次未专项验收 Patch 审批、交互卡片按钮、额外目录授权或其他网络权限请求。

本机 `codex sandbox` 的命令写法是 `codex sandbox -- COMMAND`，不是旧文档中的 `codex sandbox linux -- COMMAND`。下面的检查无须调用模型。
...
- `BRIDGE_PROFILE=codex-approval` 使用 `app_server` / `stdio://` 后端，`suggest` 模式映射为只读沙箱与 `on-request` 审批。
- 工作目录为 `/root/autodl-tmp/feishu-agent-demo/codex`。
...
创建一份权限为 600 的临时 TOML，只填写目标项目与平台。此时先不启动 bridge，运行以下命令。
...
流程依据 [cc-connect 固定版本飞书接入](https://github.com/chenhg5/cc-connect/blob/v1.5.0/docs/feishu.md)、[OpenAI 设备码认证](https://learn.chatgpt.com/docs/auth#login-on-headless-devices)，以及本次服务器命令与私密日志。
